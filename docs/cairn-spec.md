...
셋 다 새 시스템이 아니라 기존 레이어(feasibility·watcher·task·people)에 얹는 렌즈. 핵심은 사전 폼 강제 금지 — 자동 강조가 기본, 입력은 선택.

### 11.0 전제 필드 (소량 추가)
events 추가:

mode          TEXT   -- in_person | remote | async  (대면/비대면/과제 분기)
events / tasks 추가:

procurement   BOOL          -- 조달 작업인가

proc_category TEXT          -- 사무용품|소모품|IT|복지|구독|대여|기타 (등록 시 enum)

proc_route    TEXT          -- 어디서 사나/빌리나 (자유 메모, nullable)

proc_rental   BOOL          -- 대여 필요(준비물일 때)
threads 추가:

domain        TEXT   -- personal | work  (칸막이; feeds는 가로지름)
people → contacts 일반화:

kind          TEXT   -- person | vendor | venue  (사람+업체+장소 동일 스키마)

### 11.1 유저 스토리

- **사용자**로서, 일정이 과제/비대면/대면 중 무엇인지 한눈에 보고 싶다
- **사용자**로서, 그 일정의 맥락·핵심·만날 사람·준비물이 강조돼 보이길 바란다
- **사용자**로서, 대면이면 장소와 이전 장소에서 갈 방법을 시간·비용·체력으로 비교받고 싶다
- **사용자**로서, 대관/예약/케이터링 같은 건 과거 경험·연락처·특이사항이 같이 떠오르길 바란다
- **사용자**로서, 정기/비정기 구입·대여를 직접 챙길 때 루트가 정리돼 있고 정기적인 건 미리 떠올려지길 바란다
- **사용자**로서, 사적으로 해본 걸 공적 일에 재사용하라고 제안받고 싶다

### 11.2 일정 브리프 (맥락·준비물)

자동 강조가 기본, 입력은 3층(강제 0).

| ID | 기능 | 상세 |
| --- | --- | --- |
| FR-BRF-01 | mode 표시·분기 | `mode`(대면/비대면/과제)를 카드·시트에 표시. 이동·장소 로직은 `in_person`에만 작동 |
| FR-BRF-02 | 맥락 자동 강조 | 일정이 속한 thread·관련 노드·직전 annotation을 자동으로 끌어와 강조 (데이터에 이미 있음, 입력 0) |
| FR-BRF-03 | 인물 브리프 | 만날 사람 + 취급 프로파일(선호·민감점·과거 일화) 자동 표시 (6장 연동) |
| FR-BRF-04 | 준비물 3층 | ① 자동(thread/과거에서 유추) ② AI 제안("발표면 노트북·어댑터?") ③ 수동 한 줄. 사전 폼 강제 금지 |

### 11.3 이동 옵션 (대면 한정) — 데이터 소스 확정

feasibility gap을 교통수단별로 펼친 것. 외부 길찾기 API를 백엔드 게이트웨이의 어댑터로 호출(PWA 직접 호출 금지, 키는 Pi에만 — Grok 프록시와 동일 패턴).

| ID | 기능 | 상세 |
| --- | --- | --- |
| FR-MOV-01 | 이동 옵션 비교 | 이전 일정/장소 → 이 장소를 교통수단별 시간·비용·체력 3축으로. 시간=API, 체력=자체 계수, 비용=일부 API+일부 직접계산(KTX운임·연료·주차, 비교용 근사면 충분) |
| FR-MOV-02 | feasibility 연동 | 선택 이동수단의 시간→gap 검사(5장), 체력 load→체력 적분. 운전=높은 load, 좌석이동=낮은 load |
| FR-MOV-03 | mode 게이팅 | `remote`/`async` 일정엔 이동 로직 미작동(노이즈 방지) |
| FR-MOV-04 | 게이트웨이 어댑터 호출 | 외부 길찾기 API는 백엔드 게이트웨이의 지도 어댑터로 호출. 수단별 어댑터 분리 가능(대중교통/자동차 다른 제공자). 키는 Pi 백엔드에만 |
| FR-MOV-05 | 무캐시 즉시 호출 | 결과를 저장·DB화하지 않고 받는 즉시 1회 사용. NOW 게이팅과 결합 — 먼 미래는 정적 추정, 임박/선택 시점에만 실시간 호출. 호출량이 무료 한도 대비 미미(단일 사용자) → 캐시 불필요 |
| FR-MOV-06 | 원본 미저장 | 이동시간 원본을 SQLite에 영구 저장하지 않고 즉시 사용·폐기 (NCP 약관 7⑪ 준수선) |
| FR-MOV-07 | 출발지 결정 (갭 게이팅) | 이동의 출발지 = 직전 연속 일정의 위치. 직전 일정 끝~이 일정 시작 사이 갭이 임계 초과면 "직접 연결 아님"으로 보고 직접 이동계산 안 함 → 출발지 기본값(집)으로 가정하거나 비움. 갭 큰 일정 쌍을 연속 이동으로 오인 금지 |
| FR-MOV-08 | 환승 자동 구성 | 출발-도착만 주면 길찾기 API가 환승 경로(예: 6호선→DMC→1000번 버스)를 알아서 구성. 사용자가 수단·환승을 사전 기록할 필요 없음. 수단 사전 고정은 오히려 더 나은 경로를 가릴 수 있어 지양 |
| FR-MOV-09 | 예측/실측 체력 분리 | 이동 기반 예측 체력 = 표시만, 저장 안 함(API 결과 파생이므로). 하루 적분엔 즉시 계산해 쓰되 영구 저장은 실측 체력(push 답 기반, 네 데이터, 약관 무관)으로. 회고·학습은 실측으로 채움 |

**제공자 선정 (확정):**

- **대중교통 = 네이버(NCP Maps) 우선** — 한국 대중교통 품질·정확도 최상위권. 캐시를 안 쓰므로 NCP의 결과저장 금지(7⑪)가 무력화 → 품질 기준으로 선택 가능. "받는 즉시 1회 사용 + 원본 미저장" 준수.
- **자동차 = 필요 시 카카오모빌리티 어댑터 추가** — 자동차 이용은 제한적 예상. 처음엔 안 붙이고 어댑터 구조만 열어둠. (카카오는 "사용자경험 개선 목적 캐시+최신유지" 조건부 캐시 허용 제공자 — 자동차 캐시가 필요해지면 그때.)
- **폴백 = ODsay**(대중교통, 개인 무료 일 1,000회) — 보조. 한 수단=주 제공자 하나 원칙으로 한도 우회 의심 회피(대중교통을 네이버+ODsay로 분산해 한도 늘리는 형태 금지).
- **시작은 네이버 대중교통 1개** — 단일 사용자에 4개 동시 유지는 과함. 어댑터 패턴으로 이후 확장.

### 11.4 조달 (개인 책임 범위)

새 시스템 아님 — watcher(정기) + task(비정기) + 준비물(대여) + contacts(업체)의 조합. 기업 구매 시스템(재고·발주·예산)은 **Cairn 밖.**

| ID | 기능 | 상세 |
| --- | --- | --- |
| FR-PROC-01 | 정기 구매 → watcher A | "매월 토너·분기 A4·구독 갱신" = 결정론 watcher(날짜 함수). 침묵하다 발주 임계 시 표면 (8장) |
| FR-PROC-02 | 비정기 구매 → task | 필요 시 생기는 항목은 task(`due` 있으면 슬롯 추천). 이벤트 트리거형은 `thread_links.triggers` |
| FR-PROC-03 | 대여 → 준비물 종속 | `proc_rental` 준비물은 그 일정에 종속. 업체·예약 리드타임은 역산(8장 역방향 계획) |
| FR-PROC-04 | 조달 카테고리 | 등록 시 `proc_category` 선택(첨부 분류표가 enum 재료). 시스템이 카테고리를 *관리*하진 않음 — 항목별 추적만 |
| FR-PROC-05 | 루트 메모 | `proc_route`에 구매/대여처 자유 메모. 반복되면 contacts로 승격 |

### 11.5 contacts (사람·업체·장소 일반화)

| ID | 기능 | 상세 |
| --- | --- | --- |
| FR-CON-01 | contacts 일반화 | `people`을 `kind`(person/vendor/venue)로 확장. 사람 취급 프로파일 = 업체 거래 프로파일 = 장소 특이사항 |
| FR-CON-02 | 거래/방문 기억 | 업체·장소도 관계 기억(과거 거래·일화·연락처·예약 리드타임·특이사항) 자동 축적 |
| FR-CON-03 | 일정 연동 | 대관/케이터링/대여 일정에 관련 contact의 과거 경험·특이사항·해야 할 것 자동 표시 |

### 11.6 사적 ↔ 공적 교차 (domain feeds)

| ID | 기능 | 상세 |
| --- | --- | --- |
| FR-DOM-01 | domain 태그 | thread에 `personal`/`work`. 오늘·뷰에서 필터로 분리해서 봄 |
| FR-DOM-02 | 도메인 가로지르는 feeds | `thread_links.feeds`를 domain 경계 너머로 허용 (사적 렌트카 경험 → 공적 차량 대여) |
| FR-DOM-03 | 교차 재사용 제안 | "이 공적 조달, 사적으로 비슷한 거 해봤어 — 루트 재사용?" 제안 후 승인제. 자동 연결 금지 |

### 11.7 엣지케이스

- **조달을 큰 기능으로 부풀림** → 금지. 일정/태스크의 *속성(procurement 플래그)*일 뿐. 부풀면 ERP로 미끄러짐
- 첨부 분류표를 통째 구현 → 금지. enum 재료일 뿐, 재고·자동발주·예산은 Cairn 밖
- 준비물 사전 폼 강제 → 금지. 자동·제안·수동 3층, 강제 0 (사전 폼 함정)
- domain 완전 격리 vs 완전 혼재 → 둘 다 나쁨. *볼 땐 분리(필터), 연결은 명시적 feeds·승인제로만* (프라이버시·정신적 경계 보존)
- remote/async에 이동·장소 표시 → 노이즈. mode 게이팅으로 차단
- contact 특이사항이 민감/부정적("그 업체 별로") → 인물 윤리(6장)와 동일: 사실 기술만, 판단·차별 표현 회피
- 슬롯 추천이 후보마다 실시간 이동조회 → 호출 폭증 위험. 후보 비교는 정적 추정, 후보 선택 그 순간만 실측 (FR-MOV-05)
- 다중 제공자로 같은 수단 한도 분산 → 약관상 한도우회 의심. 한 수단=주 제공자 하나
- API 다운/무응답 → 이동 옵션은 "확인 불가"로 비우고 결정론 기능은 정상(격리 원칙). 정적 거리 추정으로 폴백 가능
- 출발지 불명(갭 큼/일정 띄엄띄엄) → 매번 물으면 부담, 가정하면 오류. 연속 일정만 자동 이동계산, 갭 큰 경우 출발지 집 기본 또는 비움 (FR-MOV-07)
- 같은 날 위치 불일치(A는 강남 끝, B는 상수 시작 + 4.5h 갭) → A끝을 B 출발지로 잡지 않음. B 출발지 별도 결정
- 예측 체력을 회고 위해 저장하고 싶은 유혹 → 금지. 회고는 실측만 (FR-MOV-09)

### 11.8 ❓ 미정 사항

- [x] **이동 데이터 소스 — 확정**: 네이버(대중교통) 무캐시 즉시호출 + 원본 미저장, 자동차는 카카오 어댑터 후속, 게이트웨이 어댑터 패턴. 캐시 불필요(사용량 미미)로 NCP 캐시금지 무력화 → 품질 기준 선택.
- [ ] 각 제공자 무료 일/월 한도 실측 확인 (네이버 NCP Maps 대중교통, ODsay 1,000/일 — 실제 호출 패턴 대입)
- [ ] "원본 미저장"의 약관 경계 — 체력 계산 입력으로 1회 사용이 NCP 7⑪의 "즉시 1회 사용" 안에 드는지 NCP 직접 확인 권장
- [ ] `proc_category` enum 최종 목록 (첨부표 기반, 개인 범위 압축)
- [ ] 준비물 AI 제안 트리거 (일정 type·키워드 기반?)
- [ ] domain 기본값 (미지정 시 personal?)

---

## 11-1. 🔦 가로지르는 관계 뷰 (Cross-cutting Relations)

준비물·지식·인물이 특정 일정에만 묶이지 않고 여러 노드·여러 thread에 걸칠 때, 그 관계를 본다. **핵심 원칙: 전체를 도식화하지 않는다.** 헤어볼 + 이탈 설계 위반 + 모바일 최악. 대신 "한 대상을 고르면 관련된 곳만 빛나는" 포커스+하이라이트.

### 11-1.0 전제 — resources 엔티티 (준비물·지식 통합)

준비물(물건)과 지식(정보)을 하나로 본다. "발표 자료"=물건+지식, "업체 연락처"=지식+인물연결. 분리하면 중복되므로 통합 엔티티로.
resources (신규, 소량):

id

name          TEXT   -- "노트북", "여권", "그 업체 견적서", "RPC 설정법"

kind          TEXT   -- item(물건) | knowledge(정보·노하우)

source_person INT    -- nullable, 이 지식/준비물이 누구에게서 왔나 (contacts FK)

note          TEXT   -- nullable
resource_links (N:M 연결):

resource_id

target_type   TEXT   -- event | thread | node

target_id

firmness      TEXT   -- hard | soft | tentative (이 연결의 확실성)

reason        TEXT   -- nullable, 왜 관련 (예: "발표라 노트북")

핵심: `source_person`이 **준비물/지식↔인물 직접 관계**(지금 없던 것). "이 정보는 김대리에게서", "이 설정은 형이 알려줌"을 표현. 기존엔 이벤트 경유 간접 추론만 가능했음.

### 11-1.1 유저 스토리

- **사용자**로서, 어떤 준비물·지식이 한 일정이 아니라 여러 스레드에 걸쳐 필요하면 그게 어디어디서 필요한지 보고 싶다
- **사용자**로서, 특정 인물이 스레드의 어느 부분과 관련 있는지 보고 싶다
- **사용자**로서, 어떤 지식·준비물이 누구에게서 왔는지(출처 인물) 기억하고 싶다
- **사용자**로서, 이걸 복잡한 전체 그래프가 아니라 필요한 부분만 가볍게 보고 싶다

### 11-1.2 기능 요구사항

| ID | 기능 | 상세 |
| --- | --- | --- |
| FR-XREL-01 | resource 승격 (자동 제안) | 같은 준비물·지식 이름이 여러 노드에 나오면 Cairn이 "이거 묶을까?" 제안 → 승인 시 `resources`로 승격 + N:M 연결. *전부 등록 금지*, 여러 곳 걸치는 것만 |
| FR-XREL-02 | 출처 인물 연결 | resource의 `source_person`으로 "이 지식/준비물은 누구에게서" 기록. 준비물·지식↔인물 직접 관계(신규) |
| FR-XREL-03 | 대상 선택 → 가로 하이라이트 | 인물/resource 하나를 고르면 *기존 thread spine 위에서* 관련 노드만 빛나고 나머지 흐려짐. 새 그래프 안 그림. FR-PPL-07(인물 필터)의 일반화 |
| FR-XREL-04 | ego-graph (요청 시만) | 구조를 꼭 봐야 하면 그 대상 중심의 *작은 이웃 그래프*(5~10 노드)만. 전체 그래프 절대 금지. 모바일 OK. thread_links 시각언어(실선/점선/흐림) 재사용 |
| FR-XREL-05 | firmness·이유 유지 | 하이라이트·ego-graph에도 연결의 firmness(실선/점선/흐림)와 reason("왜 관련")을 표시. 관계만 그리고 확실성·이유 잃기 금지 |

### 11-1.3 데이터 요구사항

- 신규: `resources`(소량), `resource_links`(N:M)
- 인물↔노드는 기존 `event_people`로 이미 가능 — 뷰만 신설, 데이터 추가 0
- 준비물↔인물 직접 관계는 `resources.source_person`이 신규 제공
- 하이라이트는 읽기 전용 쿼리(저장 없음)

### 11-1.4 엣지케이스

- **전체 도식화 시도** → 금지. 헤어볼 + 이탈 설계 위반. 항상 단일 대상 포커스 또는 ego-graph만
- 모든 준비물을 resource로 승격 → 관리 부담. 여러 곳 걸치는 것만, 자동 제안+승인. 일회성은 일정 종속 유지 (FR-XREL-01)
- 수동으로 일일이 연결 강제 → 죽음. 자동 추론(같은 이름 반복) 위주, 사용자는 승인만
- 하이라이트를 별도 탭으로 신설 → 표면 증식. 기존 thread spine·인물 디렉토리 위 인터랙션으로 얹음
- 연결 firmness 무시하고 다 실선 → 추론을 확정으로 위장(제6원칙 위반). soft 연결은 점선
- source_person이 불확실 → 비워둠(nullable). 환각 금지

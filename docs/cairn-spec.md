...
- **남이 만든 것** (회사 미팅·받은 초대) → GCal에 먼저 생김 → CAIRN가 읽어옴 (`source=GCAL`, `self_imposed=0`). GCal이 입구.
...
- [x] 스택 — 확정: PWA + Mac Mini 상주 로컬 서버 + SQLite 상주 (10장). LLM 라우팅·접속 경로 세부는 10.3
...
- **사용자**로서, 외부 약속은 직접 입력 없이 캘린더에서 자동으로 들어왔으면 한다
...
| 경로 | 대상 | 입력 비용 |
| --- | --- | --- |
| 자동 유입 | 외부 약속 (미팅 등) | 0 — GCal에서 당겨옴 (`source=GCAL`) |
| 한 줄 | 단발 작업·할 일 | 낮음 — "내일 3시 치과" → 이벤트 1 |
| 자연어 생성 | 구조 있는 프로젝트/여행 | 중 — 2장 생성→편집 흐름 |
...
| ID | 기능 | 상세 |
| --- | --- | --- |
| FR-SYNC-01 | GCal 이벤트 유입 | 외부 캘린더 이벤트를 `events`로 동기화 (`source=GCAL`, `self_imposed=0`) |
| FR-SYNC-02 | 한 줄 이벤트 생성 | 자연어 한 줄 → 단일 event 파싱·등록 |
...
- GCal에서 삭제된 *외부* 이벤트 → 로컬 status를 CANCELLED로 둘지, 삭제할지 결정 필요
...
**금지 패턴**: 표면 그릴 때마다 전체 상태를 LLM에 밀어넣기. 표면 집계도 결정론이고 LLM은 *변환·서술*에만 호출. 라우팅: 일상 작업(push 파싱·짧은 서술)은 로컬 LLM(Mac Mini/Gemma) → 일상 비용 ≈ 0, 프런티어 API는 무거운 추론에만.

### 9.4 ❓ 전역 미정

- [ ] 인증/계정 (단일 사용자 로컬 vs 클라우드 동기)
- [ ] 데이터 백업·내보내기 (SQLite 파일 복사 기본)
- [ ] LLM 비용 상한·로컬/API 라우팅 정책
...
| 레이어 | 선택 | 비고 |
| --- | --- | --- |
| 프런트엔드 | **PWA** (React) | 모바일 70% — 설치형 웹앱, 홈 추가·오프라인 셸·푸시. 네이티브 앱스토어 우회 |
| 백엔드 | **Mac Mini 상주 로컬 서버** | 항상 켜진 1인용 서버. cron·sync·LLM 라우팅·push 발송 전부 여기 |
| DB | **SQLite** (Mac Mini 로컬 파일) | source of truth. 백업 = 파일 복사 |
| LLM | 로컬(Gemma, Mac Mini) + 프런티어 API | 일상(push 파싱·짧은 서술)=로컬, 무거운 추론(thread 생성)=API |
| 외부 연동 | GCal API(유입), Gmail API(금전 파싱), 텔레그램(push 채널 후보) | |
| watcher B 수집 | n8n 등 외부 파이프라인 | v2, A 검증 후 (8.7) |

### 10.2 배치 함의

- **SQLite·서버가 Mac Mini에 상주** → PWA는 *클라이언트*, 데이터·연산은 서버. PWA는 서버 API를 호출.
- **접근 경로**: 집 밖에서 폰 PWA가 Mac Mini에 닿아야 함 → 고정 접속(터널/VPN/포트포워딩) 필요. ❓ 방식 미정.
- **오프라인**: PWA는 셸·최근 데이터 캐시로 읽기는 오프라인 가능, 쓰기는 서버 복귀 시 동기. ❓ 충돌 처리 정책.
- **로컬 LLM이 일상 토큰을 흡수** → 일상 비용 ≈ 0(전기), API는 가끔. 9.3과 일치.
...
- [ ] Mac Mini 다운 시 폴백 (읽기 전용 캐시 유지 정도)
- [ ] 로컬 LLM 모델 확정 (Gemma 버전·양자화)

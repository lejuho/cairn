...
**needs-review는 FR-FEAS-11 배치 규칙 따름**
...
## 5-3 전환 비용 · 순서 최적화 (Context-Switch & Sequencing)

체력 모델을 작업 하나에서 시퀀스로 끌어올린다. 핵심: 전환 비용을 별도 데이터 없이 thread_links 거리의 함수로 정의 → 체력 적분과 순서 최적화에 동시에 쓴다.

### 전환 비용 정의:
```
연속 작업 A→B:
  같은 thread              → 전환비용 0
  thread_links로 인접(contains/shares/feeds) → 낮음 (맥락 공유)
  관계 없음(엣지 없음)      → 높음 (full 컨텍스트 스위치)
  + AI needs-review 인터럽트(밀고 들어온 전환) → 추가 가산
load_total(하루) = Σ(작업 load) + Σ(전환비용)
```


| ID | 기능 | 상세 |
| --- | --- | --- |
| FR-FEAS-08 | 전환 비용 산정 |  연속 작업 쌍의 전환 비용 = thread_links 거리 함수(같음=0 / 인접=낮음 / 무관=높음). 인터럽트성 전환은 가산 |
| FR-FEAS-09 | 시퀀스 체력 적분 | 하루 체력 running sum에 작업 load + 전환 비용 합산. 같은 작업 묶음이라도 순서에 따라 하루 총합이 달라짐 (A,A,B,B < A,B,A,B) |
| FR-FEAS-10 | 순서 최적화 | DAG(links blocks/requires) 위상정렬 + 임계경로 + 병렬가능집합을 결정론으로 도출. 최적 순서 = 의존성 지키며 전환 최소 + 체력 적분 안 터지게 (전환비용을 비용 항으로) |
| FR-FEAS-11 | AI 검토 배치 | needs-review를 아무 때나 띄우지 않고 전환비용 낮은 틈(같은/인접 thread 작업 중)에 묶어 표면화. 단 적시성 상한 둠 |

### 비고 — 쪼개기는 Cairn 밖:
 작업 분해(어떻게 쪼개야 병렬 잘 되나)는 작업 내용·결합도에 달려 스키마 밖이며, planner(P/E/V)의 일. Cairn은 쪼개진 노드의 배치·순서·전환·체력만 최적화. 관심사 분리.

### 엣지케이스:

- 전환비용 계수 콜드스타트 = 순 추측 → 실측(전환 많은 날 방전 여부)으로 보정 전엔 과신 금지. "표본 부족" 표시
- 최적 순서 자동 강제 → 금지. 그날 기분·즉흥성을 무시하는 폭군이 됨. 제안("이 순서가 전환 적음")이지 강제 아님 (제1원칙)
- 검토 배치 vs 적시성 트레이드오프 → 너무 묶으면 검토 지연 → AI 폭주(Andon 늦음). "needs-review N개 쌓이거나 M시간 경과 시 전환비용 무시하고 띄움" 상한 필요
- 전환비용 산정이 가짜 정밀도로 흐름 → 방향·상대 크기까지만, 정확한 점수 환산은 신뢰도 낮으면 비표시
...
| FR-MIR-09 | 전환 횟수 기록 | 하루 thread 전환 횟수를 friction 데이터와 함께 기록. "전환 많은 날 더 방전됐나"의 보정 신호 → FR-FEAS-08 계수 튜닝 |

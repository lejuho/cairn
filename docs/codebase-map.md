...
- [web/src/MirrorLedger.tsx](/home/pi/cairn/web/src/MirrorLedger.tsx)
  - `/mirror` screen (B-temperature reflection surface). Fetches `/api/mirror/ledger` and `/api/mirror/patterns` in parallel. Five states: loading skeleton, quiet (`아직 기록된 이동/취소 원장이 없어` — when `patterns.totals.annotations===0`), live (pattern section + summary chips + newest-first ledger entries), error (retry), access_session_required (reload). Pattern section (`MirrorPatterns`): weekday/type/thread buckets with descriptive counts (`{label} 기록 N건 중 이동/취소/지각 M건`), low-sample note per section. Ledger section: entry cards with outcome 이동/취소, split cost chips, reason text/tags, logged date, optional thread link. Descriptive copy only — no advice/moralizing. Semantic tokens only (`--moved`, `.warm`).

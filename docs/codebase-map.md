...
- `server/src/repositories/*.ts`
  ...
  - [server/src/repositories/mirror.ts](/home/pi/cairn/server/src/repositories/mirror.ts) — `findMovedCancelledAnnotations(db) → MirrorSourceRow[]`: read-only LEFT JOIN annotations→events→threads for `outcome in (moved, cancelled)`, ordered `logged_at` desc then id desc. Event/thread columns nullable (join may miss). Date-range filtering is deferred to the pure service.
...
- [shared/src/mirror.ts](/home/pi/cairn/shared/src/mirror.ts)
  - Mirror Ledger A contract. `MirrorLedgerQuerySchema` (optional `from`/`to` `YYYY-MM-DD`, refine from<=to), `MirrorLedgerCostSchema` (`.strict()` — money/social/effort bucket/window/hasAnyCost; rejects an injected scalar score), `MirrorLedgerEntrySchema`, `MirrorLedgerSummarySchema` (counts + money/social totals + `effortBreakdown`), `MirrorLedgerDataSchema` (range/summary/entries/`sampleStatus` ok|low_sample), `MirrorLedgerResponseSchema`. Enums: `MIRROR_OUTCOMES` (moved|cancelled), `EFFORT_BUCKETS`, `MIRROR_SAMPLE_STATUSES`.
...
- [web/src/MirrorLedger.tsx](/home/pi/cairn/web/src/MirrorLedger.tsx)
  - `/mirror` screen (B-temperature reflection surface). Five states: loading skeleton, quiet (`아직 기록된 이동/취소 원장이 없어`), live (summary chips + newest-first ledger entries), error (retry), access_session_required (reload). Shows low-sample note (`표본이 적어 패턴으로 보긴 이르다`) when `sampleStatus==="low_sample"`. Entry cards show outcome 이동/취소, split cost chips (money/social/effort/window), reason text/tags, logged date, optional thread link. Descriptive copy only — no advice/moralizing. Semantic tokens only (`--moved`, `.warm`).

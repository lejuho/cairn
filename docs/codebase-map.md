...
- [server/src/services/feasibility.ts](/home/pi/cairn/server/src/services/feasibility.ts)
  - Pure deterministic feasibility computation: energy load (duration-hours sum), adjacent gap classification (ok/tight/impossible), near/planning mode, continuous span check. No LLM dependency. Exports `DEFAULTS` (energyBudget=8, meetBuffer=15, deepBuffer=30, travelMargin=1, maxContinuous=600). `computeDayFeasibility(date, now, events, p, relations=[])` also returns `transitionCosts` via `computeTransitionCosts` (cycle-41 FR-FEAS-08); the optional `relations` (thread_links among day threads) defaults to `[]` — callers that read only energy/continuous (slotCandidates, mirror-energy-trends) omit it and ignore the resulting transitionCosts. `dayThreadIds(events, date)` returns distinct positive thread ids among the day's scheduled planned/confirmed events (mirrors the internal `scheduled` filter) so routes load exactly the relations the service considers.

...
- [server/src/services/scheduleBrief.ts](/home/pi/cairn/server/src/services/scheduleBrief.ts)
  - Pure deterministic schedule-brief assembly (cycle-44 FR-BRF, no DB/LLM/external/movement). `buildScheduleBrief(event, thread, previousEvent, previousAnnotation, people) → ScheduleBrief`: shapes already-loaded rows into the brief; `toBriefPerson` surfaces only authored profile fields (preferredWindows/leadTime/hardConstraints → factual arrays, no advice/inference). reasonCodes `brief_*` in fixed order (mode/thread/previous_event/previous_annotation/people). `pickNewestAnnotation(annotations)` picks newest by loggedAt desc, id desc (null when empty). The route supplies the rows; this stays presentation-free (UI formats Korean copy). Repo helper `findNearestPriorThreadEvent` (repositories/events.ts) loads the prior event via offset-safe rfc3339ToMs compare.
...
- [shared/src/eventDetail.ts](/home/pi/cairn/shared/src/eventDetail.ts)
  - `CompactThreadSchema`, `EventDetailDataSchema` (event+people+annotations+thread + required `scheduleBrief` since cycle-44), `PatchEventStatusRequestSchema`, `PatchEventStatusResponseDataSchema`. `ScheduleBriefSchema` (.strict() — `mode` EventMode|null, `thread` {id,name,goal,deadline}|null, `previousEvent` {id,title,start,end}|null, `previousAnnotation` AnnotationRow|null, `people` array of authored-fact items {personId,name,relation,preferredWeekdays,preferredPeriods,leadTimeDays,unavailableWeekdays}, `reasonCodes`; rejects injected movement/procurement/advice/recommendation fields). Read-only highlight layer from data Cairn already owns; no LLM/external/movement. Types: `ScheduleBrief`, `ScheduleBriefThread`, `ScheduleBriefPreviousEvent`, `ScheduleBriefPerson`.

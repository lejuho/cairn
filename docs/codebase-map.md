...
- [web/src/PersonDetail.tsx](/home/pi/cairn/web/src/PersonDetail.tsx)
  ...
  - 작은 관계 section (cycle-40 FR-XREL-05): tap-only ego graph. "보기" button (`data-testid="person-ego-btn"`) fetches `GET /api/relations/ego?targetType=person&targetId=:id` on demand only (never on page load — asserted in tests). States closed|loading|open|error. Open renders `EgoGraphData.nodes` (center excluded) as plain `<ul>`/`<li>` list (`data-testid="person-ego-node"`): Korean type label (리소스/사람/이벤트/작업/스레드), label (href→link else span), sublabel, connecting edge firmness chip. "일부만 표시" when truncated; "연결된 항목 없음" when no neighbors. No SVG/canvas/force layout. No LLM.
...
- [web/src/Thread.tsx](/home/pi/cairn/web/src/Thread.tsx)
  ...
  - 작은 관계 보기 (cycle-40 FR-XREL-04): `ResourceFocusDetail` (the expanded panel for one selected resource) has a tap-only "작은 관계 보기" button (`data-testid="ego-open-btn"`) that fetches `GET /api/relations/ego?targetType=resource&targetId=` on demand only (never on load — asserted in tests). `EgoSheet` renders `EgoGraphData.nodes` (center excluded) as plain `<ul>`/`<li>` (`data-testid="ego-node"`): Korean type label, label (href→link else span), sublabel, edge firmness chip; "(일부만 표시)" when truncated; "연결된 항목 없음" when no neighbors. On failure shows "관계 정보를 불러올 수 없습니다." No SVG/canvas. No LLM.
